...
#trova
def post_adapter_lsa(post_adp,bigram_mdl):#nel file vecchio non serve vettorizzare il post, o almeno non lo fa quindi non so
    
    sp = spacy.load('en_core_web_sm')
    all_stopwords = sp.Defaults.stop_words
    wnl = WordNetLemmatizer()

    #post_adp = ftl.traduci(post_adp)
    #post_adp =ftl.converti_emoji(post_adp)
    
    tokens = list(gensim.utils.tokenize(remove_link_menzione(ftl.converti_emoji(ftl.traduci(post_adp))), lower=True))
    #post_adp = find_and_merge_not(tokens,all_stopwords)
    lemmatized_string=[wnl.lemmatize(words) for words in find_and_merge_not(tokens,all_stopwords)]
...
def tweet_adapter(tweet,vectorizer,bigram_mdl):
    ...
    lista_token = list(gensim.utils.tokenize(remove_link_menzione(ftl.converti_emoji(ftl.traduci(tweet))), lower=True))
    #post_adp = find_and_merge_not(tokens,all_stopwords)
    lemmatized_string=[wnl.lemmatize(words) for words in find_and_merge_not(lista_token,all_stopwords)]
    tweet_clear=bigram_mdl[lemmatized_string] 

    return vectorizer.transform([Counter(tweet_clear)])
